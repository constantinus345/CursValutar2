"""
& d:/Python_Code/ActeLocale/Scripts/python.exe d:/Python_Code/ActeLocale/RSAL_ALL_DFs.py
"""
...
#codesrsal_list = codesrsal(engine).sort()
...
for apl_cod in codesrsal_list:
    print(f"Processing >{codesrsal_list.index(apl_cod)}< out of >{len(codesrsal_list)}<")
    
    #IF Interrupted___ 
    if codesrsal_list.index(apl_cod) < 916:
        continue
    #_______________
